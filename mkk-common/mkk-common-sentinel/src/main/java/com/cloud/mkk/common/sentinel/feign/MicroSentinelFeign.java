 ...
 * @date 2020-12-14
 */
public final class MicroSentinelFeign {

	private MicroSentinelFeign() {
			...
			super.invocationHandlerFactory(new InvocationHandlerFactory() {
				...
				public InvocationHandler create(Target target, Map<Method, MethodHandler> dispatch) {
					...
					if (void.class != fallback) {
						...
						return new MicroSentinelInvocationHandler(target, dispatch,
								new FallbackFactory.Default(fallbackInstance));
					}
					if (void.class != fallbackFactory) {
						fallbackFactoryInstance = (FallbackFactory) getFromContext(beanName, "fallbackFactory",
								fallbackFactory, FallbackFactory.class);
						return new MicroSentinelInvocationHandler(target, dispatch, fallbackFactoryInstance);
					}
					return new MicroSentinelInvocationHandler(target, dispatch);
				}
